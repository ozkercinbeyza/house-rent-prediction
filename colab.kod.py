...
"""Regresyon modeli, şu formüle göre tahmin yapar:

🎯 Kira = (Size × Katsayısı) + (Bathroom × Katsayısı) + (BHK × Katsayısı) + Sabit Terim

Her özelliğin kira üzerindeki etkisi, katsayı (coefficient) ile çarpılır.
...
# Simülasyon: Gerçek ve tahmin kira değerlerini içeren örnek veri (inf olmadan)
np.random.seed(42)
gercek_kira = np.random.randint(5000, 50000, 50)
tahmin_kira = gercek_kira + np.random.normal(0, 5000, 50).astype(int)

# DataFrame oluştur
df = pd.DataFrame({
    "Gerçek Kira": gercek_kira,
    "Tahmin Edilen Kira": tahmin_kira
})

# Plot
plt.figure(figsize=(10, 6))
sns.scatterplot(x="Gerçek Kira", y="Tahmin Edilen Kira", data=df, color="#4CAF50", s=80)
plt.plot([df["Gerçek Kira"].min(), df["Gerçek Kira"].max()],
         [df["Gerçek Kira"].min(), df["Gerçek Kira"].max()],
         color='gray', linestyle='--', label='Gerçek = Tahmin')
plt.xlabel("Gerçek Kira (₹)")
plt.ylabel("Tahmin Edilen Kira (₹)")
plt.title("🎯 Gerçek vs. Tahmin Edilen Kira")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
